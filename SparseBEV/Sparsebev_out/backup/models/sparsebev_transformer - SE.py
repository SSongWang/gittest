...
#修改
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_layers=6, num_levels=4, num_classes=10, code_size=10,group_Q=True, group_K=False, q_hashes=32, k_hashes=8, pc_range=[], init_cfg=None):
        ...
        self.decoder = SparseBEVTransformerDecoder(embed_dims, num_frames, num_points, num_layers, num_levels, num_classes,code_size, group_Q,group_K, q_hashes, k_hashes, pc_range=pc_range)
...
#修改
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_layers=6, num_levels=4, num_classes=10, code_size=10,group_Q=True, group_K=False, q_hashes=32, k_hashes=8, pc_range=[], init_cfg=None):
        ...
        # params are shared across all decoder layers 修改
        self.decoder_layer = SparseBEVTransformerDecoderLayer(
            embed_dims, num_frames, num_points,  num_levels, num_classes, code_size, 2, 2,group_Q, group_K,
            q_hashes, k_hashes, pc_range=pc_range)
...
#修改
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_levels=4, num_classes=10, code_size=10, num_cls_fcs=2,
                 num_reg_fcs=2, group_Q=True, group_K=False, q_hashes=32, k_hashes=8, pc_range=[], init_cfg=None):
        ...
        #修改
        self.self_attn = AdaMultiheadAttention(embed_dims, num_heads=8,group_Q=group_Q, group_K=group_K, q_hashes=q_hashes, k_hashes=k_hashes, dropout=0.1)
    ...
    def forward(self,query_bbox, query_feat, mlvl_feats, attn_mask, img_metas):
        ...
        query_feat = self.self_attn(q, k, query_feat, attn_mask=attn_mask)[0] + query_pos


        query_feat = self.norm1(self.self_attn(q, k, query_feat, attn_mask=attn_mask)[0])
...
#         #修改
#     def __init__(self, embed_dims=256, num_heads=8, dropout=0.1,group_Q=True, group_K=False, q_hashes=32, k_hashes=32):
#         super().__init__(init_cfg)
#         #self.pc_range = pc_range
#
#         #self.attention = MultiheadAttention(embed_dims, num_heads, dropout, batch_first=True)
#         #修改
#         self.attention = AdaMultiheadAttention(embed_dims, num_heads, group_Q, group_K, q_hashes, k_hashes, dropout=dropout)
#
...
#
#     # def inner_forward(self,src, query_bbox, query_feat, pre_attn_mask):
...
#         return self.attention(q, k, src, attn_mask=src_mask,
...
class AdaptiveMixing(nn.Module):
    ...
    def inner_forward(self, x, query):
        ...
        M = SEWeightModule(self.eff_out_dim)
